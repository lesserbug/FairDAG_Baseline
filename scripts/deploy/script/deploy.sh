...
do
  ...
  if [[ -n ${FAIRDAG_CLIENT_RATE} && ${idx} -gt $((${#iplist[@]}-${client_num})) ]]; then
    benchmark_env="FAIRDAG_CLIENT_RATE=${FAIRDAG_CLIENT_RATE} FAIRDAG_SEND_DURATION=${FAIRDAG_SEND_DURATION} FAIRDAG_WARMUP_DURATION=${FAIRDAG_WARMUP_DURATION} FAIRDAG_BURST_HZ=${FAIRDAG_BURST_HZ} FAIRDAG_MAX_BATCH_DELAY_MS=${FAIRDAG_MAX_BATCH_DELAY_MS} FAIRDAG_TX_SIZE=${FAIRDAG_TX_SIZE}"
  fi
